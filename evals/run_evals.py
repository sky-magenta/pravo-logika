...
SKILL_FILES = [
    ...
    "references/time-and-conditions.md",
]
...
MODE_INSTRUCTIONS = {
    ...
    "norm": (
        "Режим РАЗБОР НОРМЫ. Возьми текст нормы ниже как ОБЪЕКТ строго по формату разбора нормы из SKILL.md "
        "(references/norm.md, §1.5): разложи на гипотезу/диспозицию/санкцию, кванторы и адресата, исключения "
        "и оговорки, модальность; назови неоднозначности В САМОЙ НОРМЕ (неопределённый признак, санкция без "
        "референта, противоречивые оговорки, круг в определении). Материю не оценивай — флаг. Если норма "
        "структурно чиста и однозначна, так и скажи."
    ),
}
